...
const API_URL =
  "https://notes-app-be-514828886605.us-central1.run.app/api/v1/notes";
...
function renderNotes(notes) {
  ...
  if (sortedNotes.length === 0) {
    notesList.innerHTML =
      '<p style="color: #888; text-align: center; margin-top: 50px;">Belum ada catatan. Buat yang pertama!</p>';
    return;
  }
  ...
  sortedNotes.forEach((note) => {
    ...
    noteCard.innerHTML = `
            <div class="note-actions">
                <button class="action-btn edit" onclick="editNote('${
                  note.id
                }', '${note.judul}', '${note.isi}')">
                    <i class="fa-solid fa-pen"></i>
                </button>
                <button class="action-btn delete" onclick="deleteNote('${
                  note.id
                }')">
